...
class FoodCategory(Model):
    ...
    def make_food_category(self, food, food_id):
        ...
        if len(food.categories) > 0:
                ...
                if len(db_cat) > 0:
                    for cat in db_cat:
                        ...
                        if name in cat_names:
                            food_has_cat.append(
                                FoodCategory({'FK_food_id': food_id,
                                              'FK_category_id': id}))
            ...
            if len(food_has_cat) > 0:
                self.bulk(food_has_cat)

    def remove(self):
        """ Remove entry on database """
        self.delete({'where': [
            ('FK_food_id', self.FK_food_id),
            ('FK_category_id', self.FK_category_id)
        ]})
